...
The rule has one honest tension in it: on a long project, never removing anything means the briefing only ever grows. The first project to really stress this ran forty versions and the briefing hit 262KB - too big to read in one pass, which defeats the point of a briefing. Auditing it showed the growth wasn't sloppiness, it was the rule working as written: each session's facts were being recorded in three to fifteen places, because there was nowhere for finished history to go. The fix wasn't to relax the rule but to give history its own home. A larger project gains an `ARCHIVE.md` beside the briefing, and content that is genuinely done - a superseded plan, a resolved question, decision prose that graduated into an ADR - moves there verbatim, leaving a one-line pointer behind. Nothing is deleted; it changes address. The briefing stays the working memory a cold session reads in one pass, the archive holds the long-form record you grep when a question actually points at it, and the `check` script warns when a briefing has grown enough to want a condense pass. Small projects never hit any of this. The condensation on that first project cut the file by three quarters with zero unique facts lost, which is about as clean a test of the split as you could ask for.
